...
if (document.getElementById('oil-selection-app')) {
	window.oilSelectionApp = new Vue({
		...
		methods: {
			...
			async renewToken() {
				const res = await (await fetch('https://api.lubribase.ru/api/v1/auth?user[email]=api-dev@lemarc.ru&user[password]=gXvJ^)9<dc&user[remote_ip]=123.0.0.1', {
					method: 'POST'
				})).json()
	...
	window.oilSelectionApp = new Vue({
		...
		methods: {
				...
				if (res) {
					...
					if (this.resultItems.length === 0) {
						...
						for (const item of this.resultItems) {
							itemData = await new Promise((res, rej) => {
								res({
									link: '/product.html',
									id: Math.random(),
									favorite: true
			...
			async getSuggestions() {
				const res = await this.request(`https://api.lubribase.ru/ru/api/v1/search_crossing`, {
					method: "GET"
				}, true)
				if (res.results) {
			...
			async renewToken() {
				const res = await (await fetch('https://api.lubribase.ru/api/v1/auth?user[email]=api-crossing-dev@lemarc.ru&user[password]=9N0MsuDNWU&user[remote_ip]=123.0.0.1', {
					method: 'POST'
				})).json()
				this.token = res.token
			},
